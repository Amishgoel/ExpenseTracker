...
const Income = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const res = await api.get(`/transactions?type=income${qs ? `&${qs}` : ""}`);
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `income-${moment().format("YYYY-MM-DD")}.xlsx`);
    toast.success("Income exported");
  };

  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-8">Income</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <h2 className="font-semibold text-slate-800 dark:text-white mb-4">Add Income</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-emerald-500 text-white py-2 rounded-lg hover:bg-emerald-600 disabled:opacity-50 font-medium"
              >
                Add Income
              </button>
            </form>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-slate-800 dark:text-white">Income History</h2>
              <p className="text-emerald-600 font-bold">Total: ₹{total.toLocaleString()}</p>
            </div>
            {loading ? (
              <p className="text-slate-500 dark:text-slate-400">Loading...</p>
            ) : filtered.length === 0 ? (
              <p className="text-slate-400 dark:text-slate-500 text-center py-12">No income recorded yet</p>
            ) : (
      ...
      {editTx && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setEditTx(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-xl w-full max-w-sm mx-4 border border-slate-200 dark:border-slate-700" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4 text-slate-800 dark:text-white">Edit Income</h3>
            <form onSubmit={handleEdit} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="flex-1 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600">Save</button>
              </div>
            </form>
          </div>
        </div>
